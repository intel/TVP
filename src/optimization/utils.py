...
def setup_e2e_optimizer_vp(model, opts):
    """model_type: str"""
...
def setup_e2e_optimizer_pad(model, opts):
    """model_type: str, one of [transformer, cnn]"""
    ...
    tp_param_optimizer = [
        (n, p) for n, p in list(model.named_parameters())
        if "tp.pad_up" in n or "tp.pad_down" in n or "tp.pad_left" in n or "tp.pad_right" in n and p.requires_grad]
    ...
    cnn_grouped_parameters = build_e2e_optimizer_w_lr_mul(
        ...
        opts.cnn_learning_rate, opts.cnn_weight_decay,
        lr_mul=opts.cnn_lr_mul, lr_mul_prefix=opts.cnn_lr_mul_prefix)
    tp_grouped_parameters = build_e2e_optimizer_w_lr_mul(
        tp_param_optimizer,
        opts.tp_learning_rate, opts.tp_weight_decay,
        lr_mul=opts.tp_lr_mul, lr_mul_prefix=opts.tp_lr_mul_prefix)
    

    optimizer_grouped_parameters = []
    optimizer_grouped_parameters.extend(trasformer_grouped_parameters)
    optimizer_grouped_parameters.extend(cnn_grouped_parameters)
    optimizer_grouped_parameters.extend(tp_grouped_parameters)
...
def setup_e2e_optimizer_txtpad(model, opts):
    """model_type: str, one of [transformer, cnn]"""

    transformer_param_optimizer = [
        (n, p) for n, p in list(model.named_parameters())
        if "transformer" in n and "text_prompt" not in n and p.requires_grad]
    cnn_param_optimizer = [
        (n, p) for n, p in list(model.named_parameters())
        if "cnn" in n and p.requires_grad]
    tp_param_optimizer = [
        (n, p) for n, p in list(model.named_parameters())
        if "tp.pad_up" in n or "tp.pad_down" in n or "tp.pad_left" in n or "tp.pad_right" in n and p.requires_grad]
    txtp_param_optimizer = [
        (n, p) for n, p in list(model.named_parameters())
        if "text_prompt" in n and p.requires_grad]
    ...
    tp_grouped_parameters = build_e2e_optimizer_w_lr_mul(
        ...
        opts.tp_learning_rate, opts.tp_weight_decay,
        lr_mul=opts.tp_lr_mul, lr_mul_prefix=opts.tp_lr_mul_prefix)
    txtp_grouped_parameters = build_e2e_optimizer_w_lr_mul(
        txtp_param_optimizer,
        opts.txtp_learning_rate, opts.txtp_weight_decay,
        lr_mul=opts.txtp_lr_mul, lr_mul_prefix=opts.txtp_lr_mul_prefix)
    ...
    optimizer_grouped_parameters.extend(cnn_grouped_parameters)
    optimizer_grouped_parameters.extend(tp_grouped_parameters)
    optimizer_grouped_parameters.extend(txtp_grouped_parameters)
...
def setup_e2e_optimizer_txt(model, opts):
    """model_type: str, one of [transformer, cnn]"""
...
def setup_e2e_optimizer_downpad(model, opts):
    """model_type: str, one of [transformer, cnn]"""
    ...
    tp_param_optimizer = [
        (n, p) for n, p in list(model.named_parameters())
        if "tp.pad_down" in n and p.requires_grad]
