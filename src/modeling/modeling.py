...
class ClipBertGroudningBaseModel(BertPreTrainedModel):
    """
    ...
    def forward(self, text_input_ids, visual_inputs, attention_mask):
        ...
        #################### 2D [B --> Different Clip Proposals]####################

        visual_embedding_output = self.visual_embeddings(visual_inputs)  # (B, Lv, d)


        visual_attention_mask = attention_mask.new_ones(
            visual_embedding_output.shape[:2])  # (B, Lv)
        
        # print(visual_attention_mask.shape[0])
        # attention_mask.repeat(visual_attention_mask.shape[0], 1)
        # a = attention_mask
        # for _ in range(visual_attention_mask.shape[0]-1):
        #     attention_mask = torch.cat([a, attention_mask], dim = 0)

        attention_mask = torch.cat(
            [attention_mask, visual_attention_mask], dim=-1)  # (B, lt+Lv, d)

        # print(text_embedding_output.shape)
        # print(visual_embedding_output.shape)
...
class ClipBertGroudningBaseModelTxt(BertPreTrainedModel):
    """
    ...
    def __init__(self, config):
        ...
        # self.text_prompt = nn.Sequential(
        #     nn.Linear(768*100, 768*200),
        #     nn.Tanh(),
        #     nn.Linear(768*200, 768*5)
        # )
    ...
    def forward(self, text_input_ids, visual_inputs, attention_mask):
        ...
        # txt_pt = self.text_prompt(text_embedding_output.view(text_embedding_output.shape[0],-1))
        # txt_pt = txt_pt.view(text_embedding_output.shape[0], 5, -1)
        ...
        #raise ValueError(f"CHECK")
        pt_mask = torch.ones(attention_mask.shape[0], 10).to(torch.float16).cuda()
        attention_mask = torch.cat(
            [pt_mask.long(), attention_mask, visual_attention_mask], dim=-1)  # (B, lt+Lv, d)
        ...
        # print(text_embedding_output.shape[0])
        # print(txt_pt.shape)
        
        # raise ValueError(f"CHECK")
        embedding_output = torch.cat(
            [txt_pt, text_embedding_output, visual_embedding_output],
            dim=1)  # (B, Lt+Lv, d)
...
class ClipBertForGrounding(BertPreTrainedModel):
    """
    Modified from BertForSequenceClassification to support oscar training.
    """
    def __init__(self, config):
        super(ClipBertForGrounding, self).__init__(config)
        self.config = config

        self.bert = ClipBertGroudningBaseModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class ClipBertForGroundingPropFree(BertPreTrainedModel):
    """
    Modified from BertForSequenceClassification to support oscar training.
    """
    def __init__(self, config):
        super(ClipBertForGroundingPropFree, self).__init__(config)
        self.config = config
        ...
        self.classifier = nn.Sequential(
            ...
            # nn.Linear(config.hidden_size * 2, config.hidden_size // 6),
            # nn.ReLU(True),
            # nn.Linear(config.hidden_size // 6, 2),
            nn.Sigmoid()
        )
        # self.classifier = nn.Sequential(
...
class ClipBertForGroundingPropFreeTxt(BertPreTrainedModel):
    """
    Modified from BertForSequenceClassification to support oscar training.
    """
    def __init__(self, config):
        super(ClipBertForGroundingPropFreeTxt, self).__init__(config)
        self.config = config

        self.bert = ClipBertGroudningBaseModelTxt(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
