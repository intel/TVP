...
from src.datasets.dataset_base import ClipBertDatasetTG


class ClipBertGroundingDataset(ClipBertDatasetTG):
    ...
    def __init__(self, datalist, tokenizer, img_lmdb_dir,
        ...
        super(ClipBertGroundingDataset, self).__init__(
    ...
    def _load_video_multi_clips_random(self, vid_id):
        ...
        for clip_idx in range(self.ensemble_n_clips):
            frames, _, duration = self._load_video(vid_id, num_clips=None, clip_idx=None)
    ...
    def _load_video_multi_clips_uniform(self, vid_id):
        ...
        video_max_pts = None
        for clip_idx in range(self.ensemble_n_clips):
            frames, video_max_pts, duration = self._load_video(
                vid_id,
    ...
    def __getitem__(self, index):
        ...
        for _ in range(num_retries):
            ...
            # print(examples[0]["time_stamp"])
            text_timestamp = torch.zeros(2, dtype=torch.float32)
            vid_duration = torch.zeros(1, dtype=torch.float32)

            text_timestamp[0] = torch.tensor(examples[0]["time_stamp"][0])
            text_timestamp[1] = torch.tensor(examples[0]["time_stamp"][1])
            vid_duration = torch.tensor(examples[0]["duration"])

            text_timestamp = examples[0]["time_stamp"]
            vid_duration = examples[0]["duration"]
            ...
            # else:
            #     if self.random_sample_clips:
            #         vid_frm_array, _, vid_duration = self._load_video(vid_id)  # tensor (T, C, H, W)
            #     else:
            #         vid_frm_array, _, vid_duration = self._load_video(vid_id, num_clips=1, clip_idx=0)  # tensor (T, C, H, W)
...
class VideoGroundingCollator(object):
    ...
    def collate_batch(self, batch):
        ...
        # durations = v_collate([d["examples"][0]["duration"] for d in batch])  # (B, )
        # timestamps = v_collate([d["examples"][0]["timestamp"] for d in batch])  # (B, 2)
        durations = v_collate([d["duration"] for d in batch])  # (B, )
        timestamps = v_collate([d["timestamp"] for d in batch])  # (B, 2)
        # group data
